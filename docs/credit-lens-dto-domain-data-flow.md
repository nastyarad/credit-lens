# Credit Lens DTO Domain Entity Data Flow
...
| History search request | `SearchFinancingRequestRequest` | full `personalIdentityCode` in the request body; `page` default `0`; `size` default `20`, allowed `1..100` |
| History search response | `SearchFinancingRequestResponse` | `FinancingRequestHistoryItemDto[]`; `page`; `size`; `totalItems`; `totalPages` |
| Details response | `FinancingRequestDetailsDto` | request metadata; `ConsumerDto`; purposes; complete `CreditExtractDto` |
| Monitoring request | `GetMonitoringFinancingRequestsRequest` | `completedFrom`; `completedTo`; `page` default `0`; `size` default `100`, allowed `1..500` |
| Monitoring response | `GetMonitoringFinancingRequestsResponse` | `MonitoringFinancingRequestDto[]`; `page`; `size`; `totalItems`; `totalPages` |
...
flowchart TB
    ...
    SEARCH[SearchFinancingRequestResponse] --> HISTORY[FinancingRequestHistoryItemDto[]]
    MONRESP[GetMonitoringFinancingRequestsResponse] --> MONITEM[MonitoringFinancingRequestDto[]]
```
...
sequenceDiagram
    ...
    loop pages of 500
        ...
        API-->>CLIENT: GetMonitoringFinancingRequestsResponse
