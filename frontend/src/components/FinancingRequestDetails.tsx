...
import { purposeLabels, voluntaryCreditBanReasonLabels } from './purposeLabels'
...
export function FinancingRequestDetailsView({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  return <section className="details-panel" aria-labelledby="details-title">
    ...
    {extract && summary && <>
      ...
      <div className={extract.voluntaryBanOnCredits.isInEffect ? 'ban-banner active-ban' : 'ban-banner'} role={extract.voluntaryBanOnCredits.isInEffect ? 'alert' : undefined}><span aria-hidden="true">{extract.voluntaryBanOnCredits.isInEffect ? '⚠' : '✓'}</span><div><strong>{extract.voluntaryBanOnCredits.isInEffect ? 'Active voluntary credit ban' : 'No active voluntary credit ban reported'}</strong>{extract.voluntaryBanOnCredits.reason && <p>Reason: {voluntaryCreditBanReasonLabels[extract.voluntaryBanOnCredits.reason]}</p>}</div></div>
      <section className="detail-section"><h2>Provenance</h2><dl className="result-grid detail-grid"><Value label="Source">Finnish Positive Credit Register</Value><Value label="Extract reference"><span title={extract.extractReference}>{extract.extractReference}</span></Value><Value label="Extract created">{instant(extract.creationTimeUtc, true)}</Value><Value label="Requested">{instant(details.requestedAt)}</Value><Value label="Completed">{instant(details.completedAt)}</Value><Value label="Purpose">{details.creditRegisterExtractPurposes.map((purpose) => purposeLabels[purpose]).join(', ')}</Value><Value label="Snapshot">Immutable</Value></dl></section>
