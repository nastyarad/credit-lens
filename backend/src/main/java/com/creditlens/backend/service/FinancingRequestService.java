...
public class FinancingRequestService {
    ...
    public FinancingRequestService(ConsumerRepository consumerRepository,
    ...
    public CreateFinancingRequestResponse create(CreateFinancingRequestRequest request) {
        PersonalIdentityCode personalIdentityCode = PersonalIdentityCode.of(request.personalIdentityCode());
        List<CreditRegisterExtractPurpose> purposes = request.creditRegisterExtractPurposes().stream()
                .map(purpose -> CreditRegisterExtractPurpose.valueOf(purpose.name())).toList();
        ...
        CreditExtract extract = positiveCreditRegisterClient.requestCreditExtract(personalIdentityCode, purposes);
        Instant completedAt = clock.instant();
        FinancingRequest saved = Objects.requireNonNull(transactions.execute(status -> {
            ConsumerEntity consumerEntity = consumerRepository.findByPersonalIdentityCode(personalIdentityCode.value())
                    .orElseGet(() -> consumerRepository.save(new ConsumerEntity(new Consumer(
                            UUID.randomUUID(), personalIdentityCode, requestedAt))));
            CreditExtract persistedExtract = extract.withPersistenceMetadata(extract.id(), completedAt);
            FinancingRequest financingRequest = FinancingRequest.create(
                    UUID.randomUUID(), request.clientRequestId(), consumerEntity.toDomain(), purposes,
                    requestedAt, completedAt, persistedExtract);
            FinancingRequestEntity requestEntity = financingRequestRepository
                    .save(new FinancingRequestEntity(financingRequest, consumerEntity));
    ...
    private FinancingRequest getFinancingRequest(FinancingRequestEntity entity) {
        CreditExtract extract = creditExtractRepository.findByFinancingRequest_Id(entity.getId())
                ...
                .orElseThrow(() -> new IllegalStateException("saved financing request has no credit extract"));
        return entity.toDomain(extract);
    }

    private CreateFinancingRequestResponse toResponse(FinancingRequest request, boolean newlyCreated) {
        CreditExtract extract = request.creditExtract();
        return new CreateFinancingRequestResponse(
                request.id(), request.clientRequestId(),
                new ConsumerDto(request.consumer().id(), request.consumer().personalIdentityCode().masked()),
                request.extractPurposes().stream().map(p -> CreditRegisterExtractPurposeDto.valueOf(p.name())).toList(),
                request.requestedAt(), request.completedAt(),
                new CreditExtractSummaryDto(extract.extractReference(), extract.creationTimeUtc(),
                        new VoluntaryBanOnCreditsDto(extract.voluntaryBanOnCredits().isInEffect(),
                                extract.voluntaryBanOnCredits().reason() == null ? null
                                        : VoluntaryCreditBanReasonDto.valueOf(extract.voluntaryBanOnCredits().reason().name())),
