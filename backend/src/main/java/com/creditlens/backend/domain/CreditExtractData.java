...
public final class CreditExtractData {
    ...
    public record PaymentPlan(boolean isInDebtArrangement, boolean isInBusinessRestructuringProgram) {
    }
    ...
    public record RunningAccountLoan(BigDecimal creditLimit, BigDecimal balance, LocalDate balanceDate) {
    }

    public record LeasingContract(LocalDate contractPeriodStartDate, BigDecimal transactionPrice) {
    }

    public record DelayedAmount(BigDecimal delayedInstalment, LocalDate originalDueDate, boolean isForeclosed) {
    }
